...
void VertexArray::addBuffer(const std::shared_ptr<VertexBuffer> vb, const std::shared_ptr<BufferLayout> bl)
{
	m_vertexBuffer = vb;
	m_bufferLayout = bl;
	this->bind();
	...
	for (unsigned int i = 0; i < elements.size(); i++) {
		...
		GL_CALL(glVertexAttribPointer(i, element.count, element.type,
			element.normalized, bl->getStride(), reinterpret_cast<const void*>(offset)));
