...
BufferLayout& BufferLayout::withLayout(const unsigned int count) {
	throw std::runtime_error(fmt::format("BufferLayout does not have implementation for 'withLayout' for type {}", typeid(T).name()));
	return *this;
}

template<>
BufferLayout& BufferLayout::withLayout<PositionWithTexture2D>() {
	m_elements.push_back({ 2, GL_FLOAT, GL_FALSE, (const void*)offsetof(PositionWithTexture2D, position) });
	m_elements.push_back({ 2, GL_FLOAT, GL_FALSE, (const void *)offsetof(PositionWithTexture2D, textureCoords)});
...
BufferLayout& BufferLayout::withLayout<PositionWithTexture3D>() {
	...
	m_elements.push_back({ 2, GL_FLOAT, GL_FALSE, (const void*)offsetof(PositionWithTexture3D, textureCoords) });
...
BufferLayout& BufferLayout::withLayout<PosNormalTexture3D>() {
	m_elements.push_back({ 3, GL_FLOAT, GL_FALSE, nullptr });
	m_elements.push_back({ 2, GL_FLOAT, GL_FALSE, (const void*)offsetof(PosNormalTexture3D, textureCoords) });
	m_elements.push_back({ 3, GL_FLOAT, GL_FALSE, (const void*)offsetof(PosNormalTexture3D, normal) });
