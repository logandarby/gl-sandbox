...
unsigned int Shader::compileShader(const unsigned int shaderType, const std::string& source)
{
	...
	if (result == GL_FALSE) {
		...
		LOG_GL_ERR("Failed to Compile {} shader: {}", (shaderType == GL_VERTEX_SHADER ? "vertex" : "fragment"), message);
	}
...
Shader& Shader::setUniform3f(const std::string& name, float v0, float v1, float v2) {
	const int location = getUniformLocation(name);
...
Shader& Shader::setLight(const std::string& name, const DirectionalLight& light)
{
...
Shader& Shader::setMaterial(const std::string& name, const T& material) {
	throw std::runtime_error(fmt::format("Shader does not have implementation for 'setMaterial' for type {}", typeid(T).name()));
	return *this;
}

template <>
Shader& Shader::setMaterial<Material>(const std::string& name, const Material& material)
{
...
template <>
Shader& Shader::setMaterial<SpecMaterial>(const std::string& name, const SpecMaterial& material)
{
	setUniform1i(fmt::format("{}.diffuse", name), material.texture);
...
Shader& Shader::setMVP(const std::string& name, const glm::mat4& model, const glm::mat4& view, const glm::mat4& projection) {
