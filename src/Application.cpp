...
Application& Application::getInstance(const int width, const int height, const std::string name)
{
	...
	m_renderer{}, m_camera{}
{
	...
	m_window.setEventCallback([&](Event& e) -> void {
		Event::dispatch<KeyPressEvent>(e, EventType::KeyPress, BIND_EVENT_FUNC(Application::keyPressCallback));
		Event::dispatch<KeyReleaseEvent>(e, EventType::KeyRelease, BIND_EVENT_FUNC(Application::keyReleaseCallback));
		Event::dispatch<MouseScrollEvent>(e, EventType::MouseScroll, BIND_EVENT_FUNC(Application::mouseScrollCallback));
		Event::dispatch<MouseOffsetEvent>(e, EventType::MouseOffset, BIND_EVENT_FUNC(Application::mouseOffsetCallback));
		});
}
...
void Application::lightingTest() {
	...
	skyboxVAO.addBuffer(
		...
		std::make_shared<BufferLayout>(BufferLayout()
			.withLayout<PositionWithTexture3D>()));
	IndexBuffer skyboxIb(skyboxCube.m_indices.data(), static_cast<unsigned int>(skyboxCube.m_indices.size()));
	...
	IndexBuffer ib(cube.m_indices.data(), static_cast<unsigned int>(cube.m_indices.size()));
	...
	lightVAO.addBuffer(
		...
		std::make_shared<BufferLayout>(BufferLayout()
			.withLayout<PositionWithTexture3D>()));
	IndexBuffer lightingIb(lightingCube.m_indices.data(), static_cast<unsigned int>(lightingCube.m_indices.size()));
	lightingCube.modelMatrix = glm::translate(lightingCube.modelMatrix, glm::vec3(2.0f, 2.0f, -2.0f));
	lightingCube.modelMatrix = glm::scale(lightingCube.modelMatrix, glm::vec3(0.6f, 0.6f, 0.6f));
	...
	m_window.whileOpen([&]() -> void {
		...
		litTextureShader
			...
			.setMVP("u_mvp", cube.modelMatrix, m_camera.getViewMatrix(), projection)
		...
		for (unsigned int i = 0; i < 10; i++) {
			...
			if (i != 0) {
				litTextureShader
					.setUniformMat4("u_mvp.model", model)
		...
		colorShader.bind();
		colorShader
			.setUniform4f("u_color", glm::vec4(light.diffuse, 1.0f))
		...
		skyboxShader.bind();
		skyboxShader
			.setMVP("u_mvp", glm::mat4(1.0f), glm::mat4(glm::mat3(m_camera.getViewMatrix())), projection)
