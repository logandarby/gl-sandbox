...
X(MouseMove) X(MouseOffset) X(MouseClick) X(MouseScroll)
...
public:
	...
	template <typename T>
	static bool dispatch(Event& e, EventType type, std::function<void(T&)> func) {
	...
	bool isInCategory(EventCategory category) const { return category & m_category; }
	std::string toString() const;
	inline bool isType(EventType type) { return m_type == type; }
protected:
	Event(EventType type, EventCategory category) : m_type{ type }, m_category{ category }, isHandled{ false } {}
...
public:
	KeyPressEvent(int keycode, int mods) : Event(EventType::KeyPress, EventCategory::KeyEvent), keycode{ keycode }, mods{ mods } {}
	const int keycode, mods;
};

class KeyReleaseEvent : public Event {
public:
	KeyReleaseEvent(int keycode, int mods) : Event(EventType::KeyRelease, EventCategory::KeyEvent), keycode{ keycode }, mods{ mods } {}
	const int keycode, mods;
};
